...
python tools/generate_quiz_bank.py `
  ...
  --model glm-4.7-flash `
  --book GEN --chapter 1 --start-verse 1 --end-verse 31
```
...
脚本默认一次发送 5 节、**严格串行**调用，适合只有 1 并发额度的模型账户。每节只收一题；模型返回的 UTF-16 下标、答案切片、词性、解释对象和无意义词规则都会在本机原文上重新验证。已有某节 5 题时默认不再请求；可用 `--max-per-verse` 调整。输出为题库格式 v2，不会写入经文文本。
